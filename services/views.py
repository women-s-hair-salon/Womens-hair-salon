...
from .utils.sms import notify_after_booking
import logging

from .models import Service, AppointmentSlot, Reservation
from .forms import DateChooseForm, TimeChooseForm
from .utils.zarinpal import ZarinpalGateway


def profile_completed_required(view_func):
    def _wrapped(request, *args, **kwargs):
        u = request.user
        if not (u.is_authenticated and getattr(u, 'is_completed', False)):
            return redirect('profile')  # نام URL پروفایل شما
        return view_func(request, *args, **kwargs)
    return _wrapped


@method_decorator([login_required, profile_completed_required], name='dispatch')
...
@method_decorator([login_required, profile_completed_required], name='dispatch')
class ChooseDateView(FormView):
    template_name = 'services/choose_date.html'
    form_class = DateChooseForm

    def dispatch(self, request, *args, **kwargs):
        self.service = get_object_or_404(Service, slug=kwargs['slug'], is_active=True)
        return super().dispatch(request, *args, **kwargs)

    def get_context_data(self, **kw):
        ctx = super().get_context_data(**kw)
        ctx['service'] = self.service
        dates = (AppointmentSlot.objects
                 .future()  # ✅ فقط تاریخ‌های آینده
                 .filter(service=self.service, is_archived=False)
                 .order_by('date')
    ...
    def form_valid(self, form):
        ...
        return redirect('choose_time', slug=self.service.slug, date=date.isoformat())


@method_decorator([login_required, profile_completed_required], name='dispatch')
class ChooseTimeView(FormView):
    template_name = 'services/choose_time.html'
    form_class = TimeChooseForm
    ...
    def get_context_data(self, **kw):
        ...
        ctx['service'] = self.service
        ctx['date'] = self.date
        slots = (AppointmentSlot.objects
                 .future()  # ✅ فقط اسلات‌های آینده
                 .filter(service=self.service, date=self.date, is_archived=False)
                 .order_by('start_time'))
        ctx['slots'] = [(s, s.remaining_capacity()) for s in slots]
        return ctx

    @transaction.atomic
    def form_valid(self, form):
        # ✅ گارد: شماره موبایل باید وجود داشته باشد
        if not getattr(self.request.user, 'phone_number', '').strip():
            return HttpResponseBadRequest('برای پرداخت، ثبت شماره موبایل الزامی است.')

        slot = get_object_or_404(
            AppointmentSlot,
            id=form.cleaned_data['slot_id'],
            service=self.service,
            date=self.date
        )
        if slot.remaining_capacity() <= 0:
            return HttpResponseBadRequest('این بازه زمانی دیگر موجود نیست.')
        ...
        r = Reservation.objects.create(
            ...
            amount_toman=self.service.deposit_amount_toman,
        )
        try:
            r.move_to_pending()
        except IntegrityError:
            return HttpResponseBadRequest('این بازه همزمان توسط شخص دیگری انتخاب شد. لطفاً بازه دیگری را انتخاب کنید.')

        # ساخت callback داینامیک
        callback_url = self.request.build_absolute_uri(reverse('payment_callback'))
        gateway = ZarinpalGateway(callback_url=callback_url)

        authority = gateway.request_payment(
            amount_toman=r.amount_toman,
            description=f"رزرو {self.service.title} — {r.time_range_label()}",
            metadata={'mobile': getattr(self.request.user, 'phone_number', '')},
        )
        r.zarinpal_authority = authority
        r.save(update_fields=['zarinpal_authority'])

        # ✅ در SANDBOX، مستقیم کال‌بک خودت را صدا بزن (بدون خروج به زرین‌پال)
        if getattr(settings, 'ZARINPAL_SANDBOX', False):
            return redirect(f"{callback_url}?Authority={authority}&Status=OK")

        # پروداکشن: ریدایرکت به StartPay واقعی
        return redirect(gateway.startpay_url(authority))

class PaymentCallbackView(TemplateView):
    template_name = 'services/payment_result.html'

    def get(self, request, *args, **kwargs):
        authority = request.GET.get('Authority')
        status = request.GET.get('Status')
        try:
            # ✅ مقاوم به رکوردهای تکراری
            r = (Reservation.objects
                 .select_related('slot', 'service', 'user')
                 .filter(zarinpal_authority=authority)
                 .order_by('-created_at')
                 .first())
        except Reservation.DoesNotExist:
            return HttpResponseBadRequest('رزرو یافت نشد')

        if status != 'OK':
            r.status = Reservation.Status.FAILED
            r.save(update_fields=['status', 'updated_at'])
            return super().get(request, *args, **kwargs)

        ok, ref_or_err = ZarinpalGateway().verify_payment(authority, r.amount_toman)
        if ok:
            r.status = Reservation.Status.BOOKED
            r.zarinpal_ref_id = str(ref_or_err)
            r.save(update_fields=['status', 'zarinpal_ref_id', 'updated_at'])
            # ✅ ارسال SMS ها
            try:
                notify_after_booking(r)
            except Exception:
                logging.getLogger(__name__).exception("SMS notify failed after booking.")
        else:
            r.status = Reservation.Status.FAILED
            r.save(update_fields=['status', 'updated_at'])
    ...
    def get_context_data(self, **kw):
        ...
        ctx['reservation'] = Reservation.objects.filter(zarinpal_authority=authority)\
                                               .select_related('slot', 'service', 'user')\
                                               .first()
        return ctx
